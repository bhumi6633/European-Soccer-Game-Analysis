...
def main():
        ...
        bmysql = tk.Button(new_window, text = "Work with MySQL", command = New_File).pack()
...
def Search_File():
  ...
  with open("Soccer.csv", 'r', newline = '') as fin:
    r = csv.reader(fin)
    pname = input("Enter player name whose record you wish to search: ")
    for i in r:
       if i[3].lower() == pname.lower():
...
def Modify_File():
  ...
  with open("Soccer.csv", 'r+', newline = '') as fin:
    r = csv.reader(fin)
    w = csv.writer(fin)
    pname = input("Enter player name whose record you wish to modify: ")
    for i in r:
       pos = fin.tell()
       if i[3].lower() == pname.lower():
          fin.seek(pos)
